...
MockActivityRepository createMockRepo({Stream<List<Activity>>? remoteStream}) {
  final repo = MockActivityRepository();

  when(
    () => repo.streamActivities(),
  ).thenAnswer((_) => remoteStream ?? Stream<List<Activity>>.empty());
...
void main() {
  ...
  group('ActivityCubit Initialization & Stream', () {
    ...
    test('starts offline sync when repository emits activities', () async {
      ...
      final activities = [
        buildActivity(id: 'planned_1', status: ActivityStatus.planned),
      ];
  ...
  group('ActivityCubit Detail & Notes Management', () {
    blocTest<ActivityCubit, List<Activity>>(
      ...
      build: () {
        final repo = createMockRepo();
        final detailedActivity = buildActivity(
          id: 'a1',
          name: 'Updated Hike Details',
        );

        when(
          () => repo.fetchActivityDetails('a1'),
        ).thenAnswer((_) async => detailedActivity);
        final cubit = ActivityCubit(
          repo,
          authChanges: () => authController.stream,
        );
    ...
    blocTest<ActivityCubit, List<Activity>>(
      ...
      expect: () => [
        isA<List<Activity>>()
            ...
            .having(
              (list) => list.first.notes.first.text,
              'note text',
              'First trail annotation',
            )
            .having(
              (list) => list.first.notes.first.id,
              'note id is not empty',
              isNotEmpty,
            ),
      ],
    );
    ...
    blocTest<ActivityCubit, List<Activity>>(
      ...
      seed: () {
        final initialNote = ActivityNote(
          id: 'n1',
          text: 'Original text',
          createdAt: DateTime(2026, 1, 1),
        );
        return [
          buildActivity(id: 'a1', notes: [initialNote]),
        ];
      },
      act: (cubit) {
        final activity = cubit.state.first;
    ...
    blocTest<ActivityCubit, List<Activity>>(
      ...
      seed: () {
        final note1 = ActivityNote(
          id: 'n1',
          text: 'To keep',
          createdAt: DateTime(2026, 1, 1),
        );
        final note2 = ActivityNote(
          id: 'n2',
          text: 'To delete',
          createdAt: DateTime(2026, 1, 1),
        );
        return [
          buildActivity(id: 'a1', notes: [note1, note2]),
        ];
      },
      act: (cubit) {
        final activity = cubit.state.first;
  ...
  test('close cancels the stream subscription', () async {
    final controller = StreamController<List<Activity>>();
    final repo = createMockRepo(remoteStream: controller.stream);
    final cubit = ActivityCubit(repo, authChanges: () => authController.stream);
    authController.add(FakeUser());
