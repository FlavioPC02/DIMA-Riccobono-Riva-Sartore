...
void main() {
  ...
  testWidgets('showLocationPermissionDialog: calls requestPermission on Enable and pops', (tester) async {
    await tester.pumpWidget(MaterialApp(home: Builder(builder: (context) {
      return ElevatedButton(
        onPressed: () => showLocationPermissionDialog(context),
        child: const Text('open'),
  ...
  testWidgets('showLocationServiceDialog: calls openLocationSettings on Enable and pops', (tester) async {
    await tester.pumpWidget(MaterialApp(home: Builder(builder: (context) {
      return ElevatedButton(
        onPressed: () => showLocationServiceDialog(context),
        child: const Text('open'),
  ...
  testWidgets('checkInitialLocation returns default map center when permission denied', (tester) async {
  ...
  testWidgets('checkInitialLocation returns current position on success', (tester) async {
    mockGeolocator.locationServiceEnabled = true;
  ...
  testWidgets('centerMapOnUser returns current center when service disabled', (tester) async {
    mockGeolocator.locationServiceEnabled = false;
    final controller = MapController();
  ...
  testWidgets('centerMapOnUser returns new position on success', (tester) async {
    mockGeolocator.locationServiceEnabled = true;
    mockGeolocator.permission = LocationPermission.always;
    final controller = MapController();
    final currentCenter = const LatLng(41.9, 12.5);
