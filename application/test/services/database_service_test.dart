...
void main() {
  ...
  test('createUser trims email and nickname and saves the correct document', () async {
    when(() => mockUserDoc.set(any(), any())).thenAnswer((_) async {});
    await databaseService.createUser('  test@example.com  ', '  First User  ');

    final capturedData = verify(() => mockUserDoc.set(captureAny(), any())).captured.single as Map<String, dynamic>;
  ...
  test('createUser throws Exception on FirebaseException', () async {
    when(() => mockUserDoc.set(any(), any())).thenThrow(FirebaseException(plugin: 'firestore'));
    await expectLater(
      databaseService.createUser('test@example.com', 'First User'),
      throwsA(isA<Exception>().having((e) => e.toString(), 'message', contains('Unable to reach the server'))),
    );
  });
  ...
  test('fetchSettings throws a StateError when the user is not authenticated', () async {
  ...
  test('streamSettings yields data', () {
    ...
    when(() => mockPreferencesDoc.snapshots()).thenAnswer((_) => Stream.value(mockSnapshot));
  ...
  test('streamProfile yields data', () {
    final mockSnapshot = MockDocumentSnapshot();
    when(() => mockSnapshot.data()).thenReturn({'nickname': 'Test'});
    when(() => mockUserDoc.snapshots()).thenAnswer((_) => Stream.value(mockSnapshot));
  ...
  test('addActivity adds a document and returns its id', () async {
    ...
    when(() => mockActivitiesCollection.add(any())).thenAnswer((_) async => mockDocRef);
  ...
  test('updateActivity updates the document', () async {
    ...
    when(() => mockActivitiesCollection.doc('act-123')).thenReturn(mockActivityDoc);
    when(() => mockActivityDoc.set(any(), any())).thenAnswer((_) async {});

    await databaseService.updateActivity('act-123', {'name': 'Updated Hike'});
    verify(() => mockActivityDoc.set({'name': 'Updated Hike'}, any())).called(1);
  });
  ...
  test('streamActivities yields mapped list of documents', () {
    ...
    when(() => mockActivitiesCollection.orderBy('date', descending: true)).thenReturn(mockQuery);
