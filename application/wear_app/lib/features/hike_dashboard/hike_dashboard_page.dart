...
import 'package:wear_app/features/hike_dashboard/widgets/compass_widget.dart';
import 'package:wear_app/features/hike_dashboard/widgets/page_chrome.dart';

class HikeDashboardPage extends StatefulWidget {
  const HikeDashboardPage({super.key});

  @override
  State<HikeDashboardPage> createState() => _HikeDashboardPageState();
}

class _HikeDashboardPageState extends State<HikeDashboardPage> {
  static const double _initialTravelledMeters = 1840.0;
  static const double _initialTrailDistanceMeters = 6200.0;
  static const double _initialWaypointMeters = 410.0;
  ...
  HikeRecordingState _recordingState = HikeRecordingState.recording;

  double _travelledMeters = _initialTravelledMeters;
  double _trailDistanceMeters = _initialTrailDistanceMeters;
  double _elevationGainMeters = 168.0;
  double _currentSpeedKmh = 4.9;
  double _waypointDistanceMeters = _initialWaypointMeters;
  double _bearingDegrees = 38.0;
  Duration _elapsed = const Duration(hours: 1, minutes: 42, seconds: 18);

  @override
  void initState() {
    super.initState();
    _ticker = Timer.periodic(const Duration(seconds: 1), _updateTelemetry);
  }
  ...
  void _updateTelemetry(Timer timer) {
    if (!mounted || !_recordingState.isRecording || _offTrailAlertVisible) {
      return;
    }

    setState(() {
      _elapsed += const Duration(seconds: 1);
      _travelledMeters = math.min(_trailDistanceMeters, _travelledMeters + 1.5);
      _elevationGainMeters += 0.28;
      _waypointDistanceMeters = math.max(0.0, _waypointDistanceMeters - 1.15);
      _bearingDegrees = (_bearingDegrees + 4.0) % 360;
      _currentSpeedKmh = 4.6 + math.sin(_travelledMeters / 190.0) * 0.55;

      if (_waypointDistanceMeters == 0.0) {
        _waypointDistanceMeters = 385.0;
      }
    });
  }
